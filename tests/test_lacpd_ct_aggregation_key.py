...
def create_lacp_state(actor_sync="1", actor_col="1", actor_dist="1",
    ...
    p = "Activ:1,TmOut:0,Aggr:1,Sync:" + partner_sync + ",Col:" + partner_col +\
    ",Dist:" + partner_dist + ",Def:0,Exp:0"
...
class lacpdAggregatoinKeyTest(OpsVsiTest):
    ...
    def test_Case1(self):
        ...
        sw_create_bond(s1, "lag200", [sw_intf[0],sw_intf_not_connected[0]], lacp_mode="active")
        set_intf_other_config(s1, sw_intf[0], ['lacp-aggregation-key=200'])
        set_intf_other_config(s1, sw_intf_not_connected[0], ['lacp-aggregation-key=200'])

        sw_create_bond(s1, "lag100", [sw_intf[1],sw_intf_not_connected[1]], lacp_mode="active")
        set_intf_other_config(s1, sw_intf[1], ['lacp-aggregation-key=100'])
        set_intf_other_config(s1, sw_intf_not_connected[1], ['lacp-aggregation-key=100'])

        #CLI(self.net)

        verify_intf_lacp_status(s1,
                                sw_intf[0],
                                create_lacp_state(actor_col="0", actor_dist="0",
                                                  partner_sync="0", partner_col="0",
                                                  partner_dist="0"),
                                "s1:" + sw_intf_str[0])
        ...
        verify_intf_lacp_status(s2,
                                sw_intf[0],
                                create_lacp_state(actor_sync="0", actor_col="0", actor_dist="0",
                                                  partner_col="0",partner_dist="0"),
                                "s2:" + sw_intf_str[0])
...
class Test_lacp_aggr_key:
    ...
    def setup_class(cls):
        Test_lacp_aggr_key.test = lacpdAggregatoinKeyTest()

        Test_lacp_aggr_key.test.net.switches[0].cmd("/bin/systemctl stop pmd")
        Test_lacp_aggr_key.test.net.switches[1].cmd("/bin/systemctl stop pmd")

    def test_Case1(self):
        self.test.test_Case1()

    def test_Case2(self):
        self.test.test_Case2()

    def test_Case3(self):
        self.test.test_Case3()

    def test_Case4(self):
        self.test.test_Case4()

    # This test case is commented because it not working
    # with the system priority and the port id
    #def test_Case5(self):
    #    self.test.test_Case5()

    def teardown_class(cls):
        Test_lacp_aggr_key.test.net.switches[0].cmd("/bin/systemctl start pmd")
        Test_lacp_aggr_key.test.net.switches[1].cmd("/bin/systemctl start pmd")

        Test_lacp_aggr_key.test.net.stop()
