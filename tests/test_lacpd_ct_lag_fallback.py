...
class lacp_fallbackTest(OpsVsiTest):
    ...
    def dynamic_lag_config(self):
        ...
        # Remove Interfaces from bond in switch 1.
        # Verify that hw_bond_config:{rx_enabled="false", tx_enabled="false"}
        # remove_intf_list_from_bond(s1, "lag0", sw_1G_intf[0:2])
        set_port_parameter(s1, "lag0", ['hw_config:rx_enabled=false'])
        set_port_parameter(s1, "lag0", ['hw_config:tx_enabled=false'])

        info("Verify that interfaces are removed from LAG"
             " when RX/TX are set to false.\n")

        # interfaces must be down due to lag failure
        info("\n### interfaces must be down due to lag failure ###\n")
        for intf in sw_1G_intf[0:2]:
            if intf == "1":
...
#               verify_intf_in_bond(s1, intf, "One Interface must be up")
        ...
        set_port_parameter(s1, "lag0", ['hw_config:rx_enabled=true'])
        set_port_parameter(s1, "lag0", ['hw_config:tx_enabled=true'])
