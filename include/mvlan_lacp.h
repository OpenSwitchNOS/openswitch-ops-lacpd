...
struct MLt_vpm_api__lacp_match_params {
    ...
    u_short actor_oper_port_number;    // Actor operational port number
    bool    is_fallback_active;        // True if fallback is active on this lport
...
typedef struct lacp_sport_params_s {
    ...
    int      intf_max_port_priority;    /* Interface with higher priority */
} lacp_sport_params_t;
