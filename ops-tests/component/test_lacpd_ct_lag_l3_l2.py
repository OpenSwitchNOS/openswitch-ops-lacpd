...
configured on interface {}'.format(first_interface)
    ...
    ip = sw1(ovs_vsctl_get_ip_cmd.format(first_interface.replace(' ', '')),
             shell='bash')
    assert ip == '"10.0.20.2/24"'
...
.format(first_interface.replace(' ', ''))
    assert expected_output in ret
    ...
    ipv6 = sw1(ovs_vsctl_get_ipv6_cmd.format(first_interface.replace(' ', '')),
               shell='bash')
