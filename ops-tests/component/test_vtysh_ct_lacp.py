...
def test_lacp_cli(topology, step):
    ...
    step('### Test LAG Load balancing for L2, L3 and L4 ###')
    sw1('interface lag 1')
    sw1('hash l2-src-dst')
    out = sw1('ovs-vsctl list port lag1', shell='bash')

    assert 'bond_mode="l2-src-dst-hash"' in out
    ...
    assert 'hash l2-src-dst-hash' in out
    ...
    assert 'bond_mode="l4-src-dst-hash"' in out
    ...
    assert "hash l4-src-dst-hash" in out
    ...
    assert re.search("(lacp * : active)", out) is not None and \
        ...
        'bond_mode="l2-src-dst-hash"' in out and \
