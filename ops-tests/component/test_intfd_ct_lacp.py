...
def test_vrf_cli(topology, step):
    ...
    step('### Testing IP address on LAG ###')
    sw1('interface lag 1')
    ret = sw1('no ip address 10.1.1.1/24')
    assert 'No IPv4 address configured on interface' in ret

    sw1('ip address 10.1.1.1/24')
    sw1("ip address 10.0.10.1/24 secondary")
    sw1("ipv6 address 2001::3/24 secondary")
    step('### Assign/remove IPv6 address to/from lag ###')
    # Adding IPv6 address to LAG
    ret = sw1('ipv6 address 2001:db8:a0b:12f0::1/64')
    assert 'Interface lag 1 is not L3.' in ret

    # Deleting IPv6 address on LAG which does
    # not have any IPv6 address
    sw1('routing')
    ret = sw1('no ipv6 address 2001:db8:a0b:12f0::1/64')
    assert 'No IPv6 address configured on interface' in ret
    ...
    assert 'ip address 10.0.10.1/24 secondary' in output[intlag_output + 2]
