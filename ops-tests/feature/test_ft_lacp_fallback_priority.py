...
##########################################################################
# Name:        test_ft_lacp_fallback_priority.py
#
# Objective:   Verify that when lacp fallback is in priority mode and partner
#              is not detected, only the interface with the highest priority
#              forwards traffic.
#
...
def test_lacp_fallback_priority(topology, step):
    """
    Case 1:
        ...
        only one interface is selected to forward traffic.
    Case 2:
        Verify that when lacp fallback is in priority mode, partner
        is not detected, and lag member interfaces have different priority
        only the interface with higher priority is selected to forward traffic.
    """
    ...
    assert lacp_map['lag' + sw2_lag_id]['fallback'] and \
        lacp_map['lag' + sw2_lag_id]['fallback_mode'] == 'priority',\
        'Fallback should be enable and in priority mode.'
