...
def test_static_dynamic_tag_vlan(topology, step):
    ...
    create_vlan(sw1, vlans_id)
    create_vlan(sw2, vlans_id)

    associate_vlan_to_lag(sw1, vlans_id, lag_id)
    associate_vlan_to_lag(sw2, vlans_id, lag_id)

    step("### Associate VLAN to L2 Interface ###")
    associate_vlan_to_l2_interface(sw1, vlans_id, ports_sw1[0])
    associate_vlan_to_l2_interface(sw2, vlans_id, ports_sw2[0])
