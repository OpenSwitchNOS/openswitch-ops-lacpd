...
def enable_switches_interfaces(sw_list, step):
    ...
    print('Waiting to ensure interfaces are turned on')
    for i in range(5):
        ...
        try:
            ...
            print('Interfaces not yet ready')
    assert False, 'Could not get interfaces up after 60 seconds wait'
...
def configure_lags(sw_list, sw_real_ports, step):
    ...
    print('Waiting 2 seconds for configuration update')
    sleep(2)
    verify_lacp_state(
        sw_list[0],
        sw_list[1],
...
def configure_workstations(hs_list, step):
    ...
    print(' '.join(
        ['Wait 15 seconds to wait for settings'
         'to establish on the workstations']
    ))
    sleep(15)
...
def change_lacp_mode(sw_list, sw_real_ports, step):
    ...
    print('Wait 5 seconds to make sure negotiation took place')
    sleep(5)
