...
from lacp_lib import APPCTL_REMOTE_STATE
...
def test_lacp_agg_key_move_interface(topology):
    ...
    sw1_lacp_state = get_appctl_lacp_state(sw1)
    sw2_lacp_state = get_appctl_lacp_state(sw2)
    map_appctl_lacp_sw1_p11 = sw1_lacp_state[str(sw1_lag_id_2)][int(p11)]
    ...
    validate_appctl_lacp_interfaces(sw1_lacp_interfaces, sw1_lag_id,
                                    [p12, p24], [p12, p24], [p12])
    validate_appctl_lacp_interfaces(sw1_lacp_interfaces, sw1_lag_id_2,
                                    [p11, p13], [p11, p13], [])
    validate_appctl_lacp_interfaces(sw2_lacp_interfaces, sw2_lag_id,
                                    [p21, p22], [p21, p22], [p22])
    ...
    validate_lag_state_default_neighbor(map_lacp_sw1_p14,
                                        LOCAL_STATE)
    # Validate appctl results
    validate_appctl_lag_state_afn(map_appctl_lacp_sw1_p11,
                                  APPCTL_LOCAL_STATE)
    ...
    validate_lag_state_sync(map_lacp_sw2_p22,
                            LOCAL_STATE)
    # Validate appctl results
    validate_appctl_lag_state_out_sync(map_appctl_lacp_sw2_p21,
                                       APPCTL_LOCAL_STATE)
