...
def test_lag_fallback(topology):
    ...
    print("Validate that one interface is up in switch 1")
    result = validate_one_interface_up(map_lacp_sw1_p11, LOCAL_STATE)
    if result:
       assert validate_one_interface_up(map_lacp_sw1_p12, LOCAL_STATE) is False,\
           "One interface must be up"
