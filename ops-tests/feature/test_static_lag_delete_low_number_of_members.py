...
def createlag(switch, lagid, configure, intarray, mode):
    if configure:
        ...
        addinterfacestolag(switch, lagid, intarray)
        if mode != 'off':
            with switch.libs.vtysh.ConfigInterfaceLag(lagid) as ctx:
...
def test_ft_static_lag_delete_low_number_of_members(topology, step):
    ...
    assert(createlag(sw2, '1', True, [sw1p2, sw1p3], 'off'))
    ...
    step("Delete LAGs")
    print("############################################")
    assert(createlag(sw1, '1', False, [], 'off'))
    assert(createlag(sw2, '1', False, [], 'off'))

    print("\n############################################")
    step("Test ping between clients does not work anymore")
    print("############################################")
    assert(pingbetweenworkstations(h1, h2, "140.1.1.11", False))

    print("\n############################################")
    step("Create LAGs with just 1 interface")
    print("############################################")
    assert(createlag(sw1, '1', True, [sw1p2], 'off'))
    assert(createlag(sw2, '1', True, [sw2p2], 'off'))

    print("\n############################################")
    step("Add VLAN configuration to newly created LAGs")
    print("############################################")
    assert(addinterfacevlan(sw1, 900, True, 'lag 1'))
    assert(addinterfacevlan(sw2, 900, True, 'lag 1'))
