...
int parse_state_from_db(const char *str, int *ret_str)
{
...
int get_lacp_state(const int *state)
{
   /* +1 for the event where all flags are ON then we have a place to store \0 */
   int ret = 0;
   int lacpActivity, lacpTimeout, aggregation, synchronization, collecting,
 ...
 if(state == NULL) return 0;
   ...
   ret |= state[0]? lacpActivity:0;
   ret |= state[1]? lacpTimeout:0;
   ret |= state[2]? aggregation:0;
   ret |= state[3]? synchronization:0;
   ret |= state[4]? collecting:0;
   ret |= state[5]? distributing:0;
   ret |= state[6]? defaulted:0;
   ret |= state[7]? expired:0;
    ...
    long *dot3adAggActorSystemPriority_val_ptr)
{
    ...
    if(temp == 0){
        temp = smap_get_int(&system_row->lacp_config, SYSTEM_LACP_CONFIG_MAP_LACP_SYSTEM_PRIORITY,
