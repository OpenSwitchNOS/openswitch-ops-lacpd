...
void delete_linux_bond(char* bond_name)
{
    ...
    sprintf(cmd_str, "%s echo -%s >  /sys/class/net/bonding_masters",
            SWNS_EXEC, bond_name);
    if (!system(cmd_str) ) {
         VLOG_DBG("bond: Failed to delete bond %s in linux", bond_name);
    }
} /* delete_linux_bond */
...
void create_linux_bond(char* bond_name)
{
    ...
    sprintf(cmd_str, "%s echo +%s >  /sys/class/net/bonding_masters",
            SWNS_EXEC, bond_name);
    if(!system(cmd_str) ) {
         VLOG_DBG("bond: Failed to create bond %s in linux", bond_name);
    }
    sprintf(cmd_str, "%s echo 2 >  /sys/class/net/%s/bonding/mode",
            SWNS_EXEC, bond_name);
    if(!system(cmd_str) ) {
         VLOG_DBG("bond: Failed to set bonding mode in bond %s", bond_name);
    }
} /* create_linux_bond */
...
void add_slave_to_bond(char* bond_name, char* slave_name)
{
    ...
    VLOG_DBG("Adding bonding slave %s to bond %s",  slave_name, bond_name);

    sprintf(cmd_str, "%s  echo +%s > /sys/class/net/%s/bonding/slaves",
            SWNS_EXEC, slave_name, bond_name);

    if (!system(cmd_str)) {
        VLOG_DBG("bond: Failed to add interface %s to bond %s", slave_name, bond_name);
    }
} /* add_slave_to_bond */
...
void remove_slave_from_bond(char* bond_name, char* slave_name)
{
    ...
    VLOG_DBG("Removing bonding slave %s from bond %s", slave_name, bond_name);

    sprintf(cmd_str, "%s  echo -%s > /sys/class/net/%s/bonding/slaves",
            SWNS_EXEC, slave_name,  bond_name);

    if (!system(cmd_str)) {
        VLOG_DBG("bond: Failed to remove interface %s from bond %s", slave_name, bond_name);
    }
...
void set_linux_bond_up(char* bond_name)
{
    ...
    if(!system(cmd_str)) {
        VLOG_DBG("bond: Failed setting bond %s to up", bond_name);
    }
} /* set_linux_bond_up */
