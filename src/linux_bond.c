...
{
    const char* load_bonding = "modprobe bonding";

    VLOG_DBG("Enabling bonding driver");

    if (!system(load_bonding)) {
         VLOG_DBG("bond: Failed to enable bonding driver in linux");
    }
} /* load_bonding_driver */
...
void delete_linux_bond(char* bond_name)
{
    ...
    VLOG_DBG("Deleting bond %s", bond_name);

    sprintf(cmd_str, "%s echo -%s >  /sys/class/net/bonding_masters",
            SWNS_EXEC, bond_name);
    if (!system(cmd_str) ) {
         VLOG_DBG("bond: Failed to delete bond in linux");
    }
} /* delete_linux_bond */
...
void create_linux_bond(char* bond_name)
{
    ...
    VLOG_DBG("Creating bond %s", bond_name);

    sprintf(cmd_str, "%s echo +%s >  /sys/class/net/bonding_masters",
            SWNS_EXEC, bond_name);
    if(!system(cmd_str) ) {
         VLOG_DBG("bond: Failed to create bond in linux");
    }
    sprintf(cmd_str, "%s echo 2 >  /sys/class/net/%s/bonding/mode",
            SWNS_EXEC, bond_name);
    if(!system(cmd_str) ) {
         VLOG_DBG("bond: Failed set bonding mode");
    }
} /* create_linux_bond */
...
void set_slave_interface_down(char* slave_name)
{
    ...
    if (!system(cmd_str) ) {
        VLOG_DBG("bond: Failed set interface to down. cmd: %s", cmd_str);
    }
...
void set_slave_interface_up(char* slave_name)
{
    ...
    if(!system(cmd_str) ) {
        VLOG_DBG("bond: Failed set interface %s to up", slave_name);
    }
...
void add_slave_to_bond(char* bond_name, char* slave_name)
{
    ...
    VLOG_DBG("Adding bonding slave %s", slave_name);

    sprintf(cmd_str, "%s  echo +%s > /sys/class/net/%s/bonding/slaves",
            SWNS_EXEC, slave_name,  bond_name);

    if (!system(cmd_str) ) {
        VLOG_DBG("bond: Failed to add interface to bonding");
    }
} /* add_slave_to_bond */
...
void remove_slave_from_bond(char* bond_name, char* slave_name)
{
    ...
    VLOG_DBG("Removing bonding slave %s", slave_name);

    sprintf(cmd_str, "%s  echo -%s > /sys/class/net/%s/bonding/slaves",
            SWNS_EXEC, slave_name,  bond_name);

    if (!system(cmd_str) ) {
        VLOG_DBG("bond: Failed to remove interface %s from bonding", slave_name);
    }
...
void set_linux_bond_up(char* bond_name)
{
    ...
    VLOG_DBG("Setting bond %s up", bond_name);

    sprintf(cmd_str, "%s ifconfig %s up", SWNS_EXEC, bond_name);
    if(!system(cmd_str) ) {
        VLOG_DBG("bond: Failed setting bond to up");
    }
} /* set_linux_bond_up */
