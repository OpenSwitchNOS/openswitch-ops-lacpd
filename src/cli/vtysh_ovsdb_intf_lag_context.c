...
| Parameters :
|     (vtysh_ovsdb_cbmsg *)p_private   : Used for idl operations
...
vtysh_intf_lag_context_clientcallback(void *p_private)
{
    ...
    data = NULL;
    data = smap_get(&port_row->other_config, PORT_OTHER_CONFIG_MAP_BOND_MODE);
    if (data) {
    ...
    data = NULL;
    data = smap_get(&port_row->other_config, PORT_OTHER_CONFIG_LACP_FALLBACK);
    if (data) {
        if (VTYSH_STR_EQ(data, PORT_OTHER_CONFIG_LACP_FALLBACK_ENABLED))
        {
            vtysh_ovsdb_cli_print(p_msg, "%4slacp fallback"," ");
        }
    }
    data = NULL;
    data = smap_get(&port_row->other_config, PORT_OTHER_CONFIG_MAP_LACP_TIME);
    if (data) {
