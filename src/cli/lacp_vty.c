...
{
  bool acls_found = false;

  ovs_assert(port_row);
  /* Check all acl columns,
   * aclv4_in_applied - indicates currently applies acls
...
lacp_add_intf_to_lag(const char *if_name, const char *lag_number)
{
   ...
   OVSREC_PORT_FOR_EACH(port_row, idl)
   {
     if(strcmp(port_row->name, if_name) == 0)
     {
        ...
        {
            vty_out(vty, "Unable to add port %s to lag %s, "
                         "ACL is configured on port.\n",if_name, lag_name);
            cli_do_config_abort(status_txn);
            /* return success since this is a business logic
