...
lacp_show_aggregates(const char *lag_name)
{
   ...
   OVSREC_PORT_FOR_EACH(lag_port, idl)
   {
          ...
          || (strcmp(lag_port->name, lag_name) == 0))
      {
         ...
         fallback_mode = smap_get(&lag_port->other_config, PORT_OTHER_CONFIG_MAP_LACP_FALLBACK_MODE);
         vty_out(vty, "%s%s%s", "Fallback mode         : ", (fallback_mode)?fallback_mode:"priority", VTY_NEWLINE);

         fallback_timeout = smap_get(&lag_port->other_config, PORT_OTHER_CONFIG_MAP_LACP_FALLBACK_TIMEOUT);
         vty_out(vty, "%s%s%s", "Fallback timeout      : ",(fallback_timeout)?fallback_timeout:"0", VTY_NEWLINE);
...
lacp_set_fallback_mode(const char *lag_name, const char *fallback_mode)
{
    ...
    if (strcmp(PORT_OTHER_CONFIG_LACP_FALLBACK_MODE_PRIORITY, fallback_mode) == 0) {
...
lacp_set_no_fallback_timeout(const char *lag_name, const char *fallback_timeout)
{
    ...
    if (!actual_fallback_timeout
        || strcmp(actual_fallback_timeout, fallback_timeout) != 0) {
        cli_do_config_abort(txn);
        return CMD_OVSDB_FAILURE;
    }
