...
lacp_remove_lb_hash_suffix(const char * lb_hash) {
  ...
  if (lb_hash) {
    ...
    temp_hash_suffix = strstr(temp_hash,"-hash");
    if (temp_hash_suffix) {
...
lacp_set_hash(const char *lag_name, const char *hash)
{
  ...
  if(strncmp("l3-src-dst-hash", hash,sizeof("l3-src-dst-hash")) == 0)
...
{
  return lacp_set_hash((char*) vty->index, "l2-src-dst-hash");
}
...
{
  return lacp_set_hash((char*) vty->index, "l2vid-src-dst-hash");
}
...
{
  return lacp_set_hash((char*) vty->index, "l3-src-dst-hash");
}
...
{
  return lacp_set_hash((char*) vty->index, "l4-src-dst-hash");
}
