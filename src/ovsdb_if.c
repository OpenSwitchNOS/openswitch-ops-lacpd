...
update_interface_bond_status_map_entry(struct iface_data *idp)
{
    ...
    if (idp->link_state == INTERFACE_LINK_STATE_UP)
    {
        ...
        if (tx_enable && rx_enable) {
            smap_replace(&smap, BOND_STATUS_UP, BOND_STATUS_TRUE);
        } else {
            smap_replace(&smap, BOND_STATUS_BLOCKED, BOND_STATUS_TRUE);
        }
    }
    else {
        /* Interface link is down */
        smap_replace(&smap, BOND_STATUS_DOWN, BOND_STATUS_TRUE);
    }
     ...
     SHASH_FOR_EACH_SAFE(node, next, &portp->cfg_member_ifs) {
        ...
        if (idp) {
            ...
            SMAP_FOR_EACH(snode, &ifrow->bond_status) {
                ...
                if (!strcmp(snode->key, BOND_STATUS_UP)) {
                    up_ifs++;
                } else if (!strcmp(snode->key, BOND_STATUS_BLOCKED)) {
                    blocked_ifs++;
                } else if (!strcmp(snode->key, BOND_STATUS_DOWN)) {
                    down_ifs++;
                }
