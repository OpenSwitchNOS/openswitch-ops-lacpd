...
*   1. port_data which contains the port data of a LAG including the list of all the configured interfaces.
*   2. lacp_per_port_variables_t which contains the pdu counters for each interface member of a lag.
* We go through all the configured interfaces for the lag specified in the parameter portp,
* and we look for an interface in lacp_per_port_vars_tree with the same port number. Once we find it
* we print the pdu counters.
*/
void lacpd_dump_pdus_per_interface(struct ds *ds, struct port_data *portp)
{
    ...
    SHASH_FOR_EACH(node, &portp->cfg_member_ifs) {
        ...
        if (idp) {
            ...
            RENTRY();
            /*Go through lacp_per_port_vars_tree looking for a lacp_per_port_variables_t corresponding to the
             * port number we found before.*/
...
* The idea of this code is to make the match between two structs:
*   1. port_data which contains the port data of a LAG including the list of all the configured interfaces.
*   2. lacp_per_port_variables_t which contains lacpd state machine control variables and the state parameters
*   for each interface member of a lag.
* We go through all the configured interfaces for the lag specified in the parameter portp,
* and we look for an interface in lacp_per_port_vars_tree with the same port number. Once we find it
* we print the lacpd state.
*/
