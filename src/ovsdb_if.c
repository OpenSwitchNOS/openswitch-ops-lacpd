...
#define OVSDB_LOCK { \
                ...
                if (!(pthread_mutex_lock(&ovsdb_mutex))) { \
                    VLOG_WARN("%s(%d): failed to  take OVSDB_LOCK lock...", __FUNCTION__, __LINE__); \
                } \
}
...
#define OVSDB_UNLOCK { \
                ...
                if (!(pthread_mutex_unlock(&ovsdb_mutex))) { \
                    VLOG_WARN("%s(%d): failed to  release OVSDB_LOCK lock...", __FUNCTION__, __LINE__); \
                } \
}
