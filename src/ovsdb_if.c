...
update_interface_bond_status_map_entry(struct iface_data *idp)
{
    ...
    if (idp->link_state == INTERFACE_LINK_STATE_UP) {
        ...
        if (tx_enable && rx_enable) {
            smap_replace(&smap, INTERFACE_BOND_STATUS_UP, INTERFACE_BOND_STATUS_ENABLED_TRUE);
        } else {
            smap_replace(&smap, INTERFACE_BOND_STATUS_BLOCKED, INTERFACE_BOND_STATUS_ENABLED_TRUE);
        }
    }
    else {
        /* Interface link is down */
        smap_replace(&smap, INTERFACE_BOND_STATUS_DOWN, INTERFACE_BOND_STATUS_ENABLED_TRUE);
    }
...
update_port_bond_status_map_entry(struct port_data *portp)
{
    ...
    if (blocked_ifs == total_ifs) {
        smap_replace(&smap, PORT_BOND_STATUS_BLOCKED, PORT_BOND_STATUS_ENABLED_TRUE);
    } else if (down_ifs == total_ifs) {
        smap_replace(&smap, PORT_BOND_STATUS_DOWN, PORT_BOND_STATUS_ENABLED_TRUE);
    } else if (up_ifs > 0) {
        smap_replace(&smap, PORT_BOND_STATUS_UP, PORT_BOND_STATUS_ENABLED_TRUE);
    }
