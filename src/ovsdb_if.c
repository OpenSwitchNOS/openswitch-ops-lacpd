    ...
    SHASH_FOR_EACH(sh_node, &all_interfaces) {
        ...
        if (OVSREC_IDL_IS_ROW_INSERTED(ifrow, idl_seqno) ||
            ...
            if (new_port_id != idp->port_id) {
                VLOG_INFO("AAA Interface %s port_id changed in DB: "
                         "new port_id=%d",
            ...
            if(key_str)
            {
                ...
                if (key != idp->actor_key) {
                    VLOG_INFO(" AAA Interface %s actor_key change in DB: "
                            "new actor_key=%d",
                            ifrow->name, key);
                    idp->actor_key = key;
                    flag = 1;
            ...
            if (mclag_enabled) {
                ...
                if ((new_sys_id == NULL && portp->sys_id != NULL) ||
                    (new_sys_id != NULL && portp->sys_id == NULL) ||
                    (new_sys_id != NULL && portp->sys_id != NULL &&
                        strcmp(new_sys_id, portp->sys_id) != 0)) {
                    if (new_sys_id == NULL) {
                        free(portp->sys_id);
                        portp->sys_id = NULL;
                        VLOG_INFO("CCCC setting sys_id to NULL");
                        set_port_overrides(portp, idp);
                    } else {
                        /* Convert the string to a mac address. */
                        eth_addr_p = ether_aton_r(new_sys_id, &eth_addr);
                        if (eth_addr_p) {
                            /* Save the system-id *after* it's been validated. */
                            free(portp->sys_id);
                            portp->sys_id = strdup(new_sys_id);
                            VLOG_INFO("CCCC setting sys_id to %s", portp->sys_id);
                            set_port_overrides(portp, idp);
                        }
                    }
                }
    ...
    SHASH_FOR_EACH_SAFE(node, next, &portp->cfg_member_ifs) {
        ...
        if (idp) {
            ...
            if (!new_mclag_enabled) {
                key_str = smap_get(&(ifrow->other_config),
                          INTERFACE_OTHER_CONFIG_MAP_LACP_AGGREGATION_KEY);
                VLOG_INFO("AAA setting key str to %s", key_str);
            } else {
                VLOG_INFO("CCCC new key");
                key_str = smap_get(&(ifrow->mclag_status),
                                   "actor_key");
            }
            ...
            if(key_str) {
                ...
                VLOG_INFO("AAA new_key is  %d", new_key);
            ...
            if (new_key != idp->actor_key) {
                ...
                VLOG_INFO("AAA setting actor key to  %d", new_key);
                changed = true;
            }
...
handle_port_config(const struct ovsrec_port *row, struct port_data *portp)
{
    ...
    if (lacp_mode != PORT_LACP_OFF) {
        ...
        if (new_mclag_enabled != portp->mclag_enabled) {
            ...
            VLOG_INFO("CCCC mclag_enabled changed");
