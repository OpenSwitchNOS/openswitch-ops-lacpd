...
static void
send_fallback_status_msg(int lag_id, bool fallback_status)
{
    ML_event *event;
    struct MLt_vpm_api__fallback_status *msg;
    int msgSize;

    VLOG_DBG("%s: lag_id=%d, fallback=%d",
             __FUNCTION__,
            lag_id,
            fallback_status);

    msgSize = sizeof(ML_event) + sizeof(struct MLt_vpm_api__fallback_status);
    ...
    if (event != NULL) {
        /*** From CfgMgr peer. ***/
        event->sender.peer = ml_cfgMgr_index;
        event->msgnum = MLm_vpm_api__set_lacp_fallback_status;
         ...
         * access it in LACP process space.
         */
        msg = (struct MLt_vpm_api__fallback_status *)(event+1);
        msg->sport_handle = PM_LAG2HANDLE(lag_id);
...
handle_port_config(const struct ovsrec_port *row, struct port_data *portp)
{
    ...
    if (lacp_mode != PORT_LACP_OFF) {
        ...
        if (!ovs_fallback) {
            if (portp->fallback_enabled) {
                /* Admin has disabled fallback */
                portp->fallback_enabled = false;
                send_fallback_status_msg(portp->lag_id, false);
            }
        } else {
            if (strncmp(ovs_fallback,
                ...
                if (!portp->fallback_enabled) {
                    /* Admin has enabled fallback */
                    portp->fallback_enabled = true;
                    send_fallback_status_msg(portp->lag_id, true);
                }
            }
        }
    }
