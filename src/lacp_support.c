...
{
    lacp_per_port_variables_t *plpinfo = NULL;
    VLOG_INFO("fall: %s", __FUNCTION__);
    plpinfo = LACP_AVL_FIND(lacp_per_port_vars_tree, &lport_handle);

    VLOG_INFO("fall: fallback params %d %d", mode, timeout);
    if (plpinfo != NULL) {
        VLOG_INFO("fall: lport %d %d", plpinfo->actor_oper_port_number, plpinfo->actor_oper_port_key);
        plpinfo->ovs_timeout = timeout;
        plpinfo->fallback_mode = mode;
    } else {
        VLOG_INFO("fall: Set lport fallback status: lport_handle 0x%llx not found", lport_handle);
        VLOG_ERR("Set lport fallback status: lport_handle 0x%llx not found",
                 lport_handle);
    }
